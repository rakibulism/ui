...
- **Prefer no new runtime dependencies.** Most components ship with `clsx` only — positioning, portals, and state are handled with React and CSS. The exception is the handful of components (`Modal`, `Menu`, `Tooltip`, `Toast`, `Tabs`, `Accordion`, `Select`) that wrap a Radix UI primitive internally for accessibility that's genuinely hard to hand-roll correctly (focus trapping, roving tabindex, collision-aware positioning) — if you're touching one of those, keep using its existing Radix primitive rather than reintroducing hand-rolled logic, and don't add a new dependency for a component that doesn't already have one without discussing it first.
